...
switch ($accion) {
    ...
    case 'agregar':
        ...
        if (empty($nombre) || $precio < 0) {
            $_SESSION['alert'] = ['icon'=>'warning','title'=>'Datos inválidos','text'=>'Los datos ingresados son inválidos. Verifique los campos señalados.'];
        ...
        if ($stmt->rowCount() > 0) {
            $_SESSION['alert'] = ['icon'=>'error','title'=>'Duplicado','text'=>'Este repuesto ya está registrado en el inventario.'];
            header("Location: ../views/dashboard/admin_inventario.php"); exit;
        }
        ...
        try {
            ...
            $db->commit();
            $_SESSION['alert'] = ['icon'=>'success','title'=>'Producto agregado','text'=>'Producto registrado exitosamente.'];
        } catch (Exception $e) {
            $db->rollBack();
    ...
    case 'actualizar':
        ...
        if ($cantidad < 0) {
            $_SESSION['alert'] = ['icon'=>'warning','title'=>'Cantidad inválida','text'=>'Los datos ingresados son inválidos.'];
            header("Location: ../views/dashboard/admin_inventario.php"); exit;
        }

        try {
            $db->prepare(
                "UPDATE inventario SET cantidad=:cantidad, stock_minimo=:stock WHERE id_inventario=:id"
